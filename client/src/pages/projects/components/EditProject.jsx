...
const EditProject = ({ project}) => {
  ...
  const editProject = useMutation(
    async (values) => {
      const response = await axios.patch(
        `http://localhost:3000/api/project/addProject/${project._id}`,
        values
      );
  ...
  return (
    <form onSubmit={formik.handleSubmit} className="flex flex-col gap-2">
      ...
      onClick={() => document.getElementById("editProject").close()}
      >
        <IoCloseSharp />
      </div>
      ...
      <button
        ...
        type="submit"
        disabled={editProject.isLoading}
      >
        {editProject.isLoading ? "Editing..." : "edit Project"}
      </button>
    </form>
  );
};
